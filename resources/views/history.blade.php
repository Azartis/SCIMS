        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            @if ($auditLogs->isEmpty())
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-gray-600 dark:text-gray-400">{{ __('No changes recorded yet.') }}</p>
                </div>
            @else
                <div class="space-y-6">
                        ...
                        <div x-data="{ open: false }" class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                            ...
                                            <span class="text-sm text-gray-500 dark:text-gray-400">
                                                ({{ $log->auditable ? $log->auditable->getFormattedDisplayName() : __('N/A') }})
                                            </span>
                                        </h3>
                                    ...
                                    {{-- toggle button --}}
                                    <button @click="open = !open" class="text-blue-600 dark:text-blue-400 text-xs">
                                        <span x-text="open ? 'Hide details' : 'Show details'"></span>
                                    </button>
                                </div>

                                <div x-show="open" class="space-y-3" x-cloak>
